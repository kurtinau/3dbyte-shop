...
export const ProductCard = ({ data, deviceType }: Props) => {
  const { title, gallery, price, salePrice, slug, discountInPercent, Color } = data;
  const colorUrl = Color.length ? Color[0].Image[0].formats.small.url : null;
    ...
    <Link href='/products/[slug]' as={`/products/${slug}`}>
      <Card>
        ...
        <Box padding={30}>
          ...
          {/* ////TODO reformate data to fit cart. */}
            {Color.length ? <PreviewButton handlePreviewClick={handlePreviewClick} /> : <AddItemToCart data={data} />}
          </Box>
        </Box>
      </Card>
    </Link >
